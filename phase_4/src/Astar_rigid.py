        ...
        # Backtracking from the goal cell to extract an optimal path
            ...
            while sum(next_cell) >= 0:
                if current_cell[2] == next_cell[2]:
                    ...
                    distance = sqrt((current_actual[0] - self.actionGrid[current_cell][0]) ** 2 +\
                                    (current_actual[1] - self.actionGrid[current_cell][1]) ** 2)
                    self.speed_points.append([distance, 0])
                else:
                    ...
                    distance = cr * (th2 - th1) * (pi / 180.0) * self.i_res
                    direction = (th2 - th1) * (pi / 180.0) * turn
                    self.speed_points.append([distance, direction])
